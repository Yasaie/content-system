...
<div class="row">
    <div class="col-lg-12">
        <div class="card-box">
            ...
            <hr>
            <form class="form" method="POST" action="{{ route('panel.menu.update',$menu) }}">
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} col-md-6">
                        <label class="control-label" for="title">عنوان</label>
                        <input id="title" name="title" class="form-control" placeholder="عنوان" value="{{ $menu->title ?? old('title') }}" type="text">
                        @if($errors->has('title'))
                            <div>
                    ...
                    <div class="form-group {{ $errors->has('label') ? 'has-error' : '' }} col-md-6">
                        ...
                        <input id="label" name="label" class="form-control" placeholder="برچسب" value="{{ $menu->label ?? old('label') }}" type="text">
                        @if($errors->has('label'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('label') }}
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('url') ? 'has-error' : '' }} col-md-8">
                        ...
                        <input id="url" name="url" class="form-control" placeholder="آدرس" value="{{ $menu->url ?? old('url') }}" type="text">
                        @if($errors->has('url'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('url') }}
                    ...
                    <div class="form-group {{ $errors->has('publish') ? 'has-error' : '' }} col-md-4">
                        ...
                        <select id="publish" name="publish" class="form-control">
                            <option {{ ($menu->published() ? 'selected' : '') }} value="1">انتشار بده</option>
                            <option {{ ($menu->notPublished() ? 'selected' : '') }} value="0">پیش نویس کن</option>
                        </select>
