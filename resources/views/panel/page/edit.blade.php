...
<ol class="breadcrumb">
    ...
    <li>
        <a href="{{ route('panel.page.index') }}">صفحات</a>
    </li>
    <li class="active">ویرایش صفحه</li>
</ol>

<div class="row">
    <form class="form" method="POST" action="{{ route('panel.page.update',$page) }}" enctype="multipart/form-data">
        <div class="col-lg-8">
            <div class="card-box">
                <h4 class="text-dark header-title m-t-0">ویرایش صفحه</h4>
                <hr>
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="title" name="title" class="form-control" placeholder="عنوان" value="{{ $page->title ?? old('title') }}" type="text">
                        @if($errors->has('title'))
                            <div>
                    ...
                    <div class="form-group {{ $errors->has('body') ? 'has-error' : '' }} col-md-12">
                        ...
                        <textarea id="body" name="body" class="form-control" placeholder="محتوا" cols="30" rows="10">{{ $page->body ?? old('body') }}</textarea>
                        @if($errors->has('body'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('body') }}
            ...
            <div class="card-box">
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('seo_title') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_title" name="seo_title" class="form-control" placeholder="عنوان در تگ title قرار میگیرد" value="{{ $page->seo->title ?? old('seo_title') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('seo_description') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_description" name="seo_description" class="form-control" placeholder="توضیحات در متا تگ description قرار میگیرد" value="{{ $page->seo->description ?? old('seo_description') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('seo_keywords') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_keywords" name="seo_keywords" class="form-control" placeholder="کلمات کلیدی در تگ meta قرار میگیرند" value="{{ $page->seo->keywords ?? old('seo_keywords') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('seo_canonical') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_canonical" name="seo_canonical" class="form-control" placeholder="لینک در متا تگ canonical قرار میگیرد" value="{{ $page->seo->canonical ??  old('seo_canonical') }}" type="text">
            ...
            <div class="card-box">
                ...
                <div class="row">
                    <div class="form-group col-md-12">
                        <label class="control-label" for="page">آدرس صفحه</label>
                        <input class="form-control text-right" id="page" value="{{ route('page.show',['id'=>$page,'title'=>'pagelink']) }}" readonly type="text">
                    </div>
                    <div class="form-group {{ $errors->has('publish') ? 'has-error' : '' }} col-md-12">
                        ...
                        <select id="publish" name="publish" class="form-control">
                            <option {{ ($page->published() ? 'selected' : '') }} value="1">انتشار بده</option>
                            <option {{ ($page->notPublished() ? 'selected' : '') }} value="0">پیش نویس کن</option>
                        </select>
                    ...
                    <div id="mainShowFields">
                        <div class="form-group {{ $errors->has('meta_mainshow') ? 'has-error' : '' }} col-md-12">
                            ...
                            <select v-model="mainShow" id="meta_mainshow" name="meta_mainshow" class="form-control">
                                <option {{ (empty($page->showInMainPage()) ? 'selected' : '') }} value="0">عدم نمایش</option>
                                <option {{ (!empty($page->showInMainPage()) ? 'selected' : '') }} value="1">نمایش دادن</option>
                            </select>
                        ...
                        <div v-if="mainShow==1" v-cloak class="form-group {{ $errors->has('meta_tabtitle') ? 'has-error' : '' }} col-md-12">
                            <label class="control-label" for="meta_tabtitle">عنوان تب</label>
                            <input id="meta_tabtitle" name="meta_tabtitle" class="form-control" placeholder="عنوان تب در صفحه اول" value="{{ $page->meta_tabtitle ??  old('meta_tabtitle') }}" type="text" required>
                            @if($errors->has('meta_tabtitle'))
                                <div>
                                    <strong class="help-block text-danger">
                                        {{ $errors->first('meta_tabtitle') }}
                                    </strong>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="form-group {{ $errors->has('media') ? 'has-error' : '' }} col-md-12">
                        ...
                        @if(!empty($page->image))
                            <div>
                                <img class="img-responsive thumbnail" src="{{ fileUploadUrl($page->image) }}">
                            </div>
                        @endif
...
@push('scripts')
    ...
    <script>
        ...
        new Vue({
            el: '#mainShowFields',
            data: {
                mainShow: {{ (!empty($page->showInMainPage()) ? 1 : 0) }}
            }
        });
    </script>
