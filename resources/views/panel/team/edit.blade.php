...
<ol class="breadcrumb">
    ...
    <li class="active">ویرایش عضو تیم</li>
</ol>

<div class="row">
    <form class="form" method="POST" action="{{ route('panel.team.update',$team) }}" enctype="multipart/form-data">
        <div class="col-lg-8">
            <div class="card-box">
                <h4 class="text-dark header-title m-t-0">ویرایش عضو تیم</h4>
                <hr>
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="title" name="title" class="form-control" placeholder="عنوان" value="{{ $team->title ?? old('title') }}" type="text">
                        @if($errors->has('title'))
                            <div>
                    ...
                    <div class="form-group {{ $errors->has('body') ? 'has-error' : '' }} col-md-12">
                        ...
                        <textarea id="body" name="body" class="form-control" placeholder="محتوا" cols="30" rows="10">{{ $team->body ?? old('body') }}</textarea>
                        @if($errors->has('body'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('body') }}
            ...
            <div class="card-box">
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('seo_title') ? 'has-error' : '' }} col-md-12">
                        <label class="control-label" for="seo_title">توضیحات در مورد نقش و رزومه کاری این فرد در تیم</label>
                        <input id="seo_title" name="seo_title" class="form-control" placeholder="عنوان در تگ title قرار میگیرد" value="{{ $team->seo->title ?? old('seo_title') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('seo_description') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_description" name="seo_description" class="form-control" placeholder="توضیحات در متا تگ description قرار میگیرد" value="{{ $team->seo->description ?? old('seo_description') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('seo_keywords') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_keywords" name="seo_keywords" class="form-control" placeholder="کلمات کلیدی در تگ meta قرار میگیرند" value="{{ $team->seo->keywords ?? old('seo_keywords') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('seo_canonical') ? 'has-error' : '' }} col-md-12">
                        ...
                        <input id="seo_canonical" name="seo_canonical" class="form-control" placeholder="لینک در متا تگ canonical قرار میگیرد" value="{{ $team->seo->canonical ??  old('seo_canonical') }}" type="text">
            ...
            <div class="card-box">
                ...
                <div class="row">
                    <div class="form-group col-md-12">
                        ...
                        <input class="form-control text-right" id="page" value="{{ route('team.show',['id'=>$team,'title'=>'pagelink']) }}" readonly type="text">
                    </div>
                    <div class="form-group {{ $errors->has('publish') ? 'has-error' : '' }} col-md-12">
                        ...
                        <select id="publish" name="publish" class="form-control">
                            <option {{ ($team->published() ? 'selected' : '') }} value="1">انتشار بده</option>
                            <option {{ ($team->notPublished() ? 'selected' : '') }} value="0">پیش نویس کن</option>
                        </select>
                    ...
                    <div id="mainShowFields">
                        <div class="form-group {{ $errors->has('meta_mainshow') ? 'has-error' : '' }} col-md-12">
                            ...
                            <select v-model="mainShow" id="meta_mainshow" name="meta_mainshow" class="form-control">
                                <option {{ (empty($team->showInMainPage()) ? 'selected' : '') }} value="0">عدم نمایش</option>
                                <option {{ (!empty($team->showInMainPage()) ? 'selected' : '') }} value="1">نمایش دادن</option>
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('meta_job') ? 'has-error' : '' }} col-md-12">
                            ...
                            <input id="meta_job" name="meta_job" class="form-control" placeholder="سمت یا نقش این فرد در تیم" value="{{ $team->meta_job??  old('meta_job') }}" type="text" required>
                    ...
                    <div class="form-group {{ $errors->has('media') ? 'has-error' : '' }} col-md-12">
                        ...
                        @if(!empty($team->image))
                            <div>
                                <img class="img-responsive thumbnail" src="{{ fileUploadUrl($team->image) }}">
                            </div>
                        @endif
...
@push('scripts')
    ...
    <script>
        new Vue({
            el: '#mainShowFields',
            data: {
                mainShow: {{ (!empty($team->showInMainPage()) ? 1 : 0) }}
            }
        });
    </script>
