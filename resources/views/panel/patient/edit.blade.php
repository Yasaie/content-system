...
@section('content')
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">ویرایش اطلاعات بیمار</li>
    </ol>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card-box">
                ...
                <hr>
                <form class="form" method="POST" action="{{ route('panel.patient.update', $patient) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }} col-md-4">
                            <label class="control-label" for="name">نام و نام خانوادگی</label>
                            <input type="text" name="name" value="{{ old('name') ?? $patient->name }}" id="name" class="form-control" placeholder="نام و نام خانوادگی">
                            @if($errors->has('name'))
                                <div>
                        ...
                        <div class="form-group {{ $errors->has('age') ? 'has-error' : '' }} col-md-4">
                            ...
                            <input type="text" name="age" value="{{ old('age') ?? $patient->age }}" id="age" class="form-control" placeholder="سن">
                            @if($errors->has('age'))
                                <div>
                                    <strong class="help-block text-danger">
                                        {{ $errors->first('age') }}
                        ...
                        <div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }} col-md-4">
                            ...
                            <select name="gender" id="gender" class="form-control">
                                <option {{ ($patient->gender == 'male' ? 'selected' : '') }} value="male">مذکر</option>
                                <option {{ ($patient->gender == 'female' ? 'selected' : '') }} value="female">مونث</option>
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }} col-md-6">
                            ...
                            <input type="text" name="phone" value="{{ old('phone') ?? $patient->phone }}" id="phone" class="form-control" placeholder="شماره تماس">
                            @if($errors->has('phone'))
                                <div>
                                    <strong class="help-block text-danger">
                                        {{ $errors->first('phone') }}
                        ...
                        <div class="form-group {{ $errors->has('mobile') ? 'has-error' : '' }} col-md-6">
                            ...
                            <input type="text" name="mobile" value="{{ old('mobile') ?? $patient->mobile }}" id="mobile" class="form-control" placeholder="شماره موبایل">
                        ...
                        <div class="form-group {{ $errors->has('postal_code') ? 'has-error' : '' }} col-md-12">
                            ...
                            <input type="text" name="postal_code" value="{{ old('postal_code') ?? $patient->postal_code }}" id="postal_code" class="form-control" placeholder="کدپستی">
                    ...
                    <div class="row">
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }} col-md-12">
                            ...
                            <textarea name="address" id="address" class="form-control" placeholder="آدرس (استان - شهرستان - خیابان - کوچه - ساختمان - طبقه - پلاک)" cols="30" rows="10">{{ old('address') ?? $patient->address }}</textarea>
