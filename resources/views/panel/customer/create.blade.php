...
<ol class="breadcrumb">
    ...
    <li class="active">ثبت مشتری جدید</li>
</ol>

<div class="row">
    <div class="col-lg-12">
        <div class="card-box">
            <h4 class="text-dark header-title m-t-0">ثبت مشتری جدید</h4>
            <hr>
            <form class="form" method="POST" action="{{ route('panel.customer.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }} col-md-6">
                        <label class="control-label" for="name">نام مشتری</label>
                        <input id="name" name="name" class="form-control" placeholder="نام" value="{{ old('name') }}" type="text">
                        @if($errors->has('name'))
                            <div>
                    ...
                    <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }} col-md-6">
                        ...
                        <input id="phone" name="phone" class="form-control" placeholder="شماره تماس" value="{{ old('phone') }}" type="text">
                        @if($errors->has('phone'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('phone') }}
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('meta_job') ? 'has-error' : '' }} col-md-6">
                        ...
                        <input id="meta_job" name="meta_job" class="form-control" placeholder="مقام یا منصب" value="{{ old('meta_job') }}" type="text">
                    ...
                    <div class="form-group {{ $errors->has('mainpage') ? 'has-error' : '' }} col-md-3">
                        ...
                        <select id="publish" name="mainpage" class="form-control">
                            <option {{ (old('mainpage')==1 ? 'selected' : '') }} value="1">نمایش بده</option>
                            <option {{ (!empty(old('mainpage')) && old('mainpage')==0 ? 'selected' : '') }} value="0">نمایش نده</option>
                        </select>
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('idea') ? 'has-error' : '' }} col-md-12">
                        ...
                        <textarea name="idea" id="idea" class="form-control" placeholder="نظر مشتری" cols="30" rows="10">{{ old('idea') }}</textarea>
                        @if($errors->has('idea'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('idea') }}
