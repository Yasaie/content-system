...
<ol class="breadcrumb">
    ...
    <li class="active">ایجاد دسته بندی جدید</li>
</ol>

<div class="row">
    <div class="col-lg-12">
        <div class="card-box">
            <h4 class="text-dark header-title m-t-0">دسته بندی</h4>
            <hr>
            <form class="form" method="POST" action="{{ route('panel.category.store') }}">
                @csrf
                <div class="row">
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }} col-md-8">
                        ...
                        <input id="slug" name="slug" class="form-control" placeholder="اسلاگ" value="{{ old('slug') }}" type="text">
                        @if($errors->has('slug'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('slug') }}
