...
<div class="row">
    <div class="col-lg-12">
        <div class="card-box">
            <h4 class="text-dark header-title m-t-0">ویرایش اسلاید</h4>
            <hr>
            <form class="form" method="POST" action="{{ route('panel.slide.update',$slide) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="row">
                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }} col-md-6">
                        <label class="control-label" for="title">عنوان</label>
                        <input id="title" name="title" class="form-control" placeholder="عنوان" value="{{ $slide->title ?? old('title') }}" type="text" required>
                        @if($errors->has('title'))
                            <div>
                    ...
                    <div class="form-group {{ $errors->has('url') ? 'has-error' : '' }} col-md-6">
                        ...
                        <input id="url" name="url" class="form-control" placeholder="آدرس" value="{{ $slide->url ?? old('url') }}" type="text">
                        @if($errors->has('url'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('url') }}
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('color') ? 'has-error' : '' }} col-md-6">
                        ...
                        <input id="color" name="color" class="form-control" placeholder="رنگ" value="{{ $slide->color ?? old('color') }}" type="color">
                        @if($errors->has('color'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('color') }}
                    ...
                    <div class="form-group {{ $errors->has('publish') ? 'has-error' : '' }} col-md-6">
                        ...
                        <select id="publish" name="publish" class="form-control">
                            <option {{ ($slide->published() ? 'selected' : '') }} value="1">انتشار بده</option>
                            <option {{ ($slide->notPublished() ? 'selected' : '') }} value="0">پیش نویس کن</option>
                        </select>
                ...
                <div class="row">
                    <div class="form-group {{ $errors->has('body') ? 'has-error' : '' }} col-md-12">
                        ...
                        <textarea name="body" id="body" class="form-control" placeholder="توضیحات" cols="30" rows="10" required>{{ $slide->body ?? old('body') }}</textarea>
                        @if($errors->has('body'))
                            <div>
                                <strong class="help-block text-danger">
                                    {{ $errors->first('body') }}
                ...
                <div class="row">
                    <div>
                        @if(!empty($slide->image))
                            <img style="width:100px;" class="thumbnail" src="{{ fileUploadUrl($slide->image) }}">
                        @endif
                    </div>
